...
const AddTouristPlace = () => {
  const [touristPlaceList, setTouristPlaceList] = React.useState([]);
  ...
  React.useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_APP_PUBLIC_SERVER}/api/touristspot`)
      .then((res) => {
        setTouristPlaceList(res.data);
      });
  }, []);
  ...
  const blogHandler = (target, method, _id) => {
    ...
    try {
      axios[method](
        `${import.meta.env.VITE_APP_PUBLIC_SERVER}/api/touristspot${_id ? `/${_id}` : ""}`,
        formData,
        {
        ...
        .then((res) => {
          if (res.status === 201) {
            if (_id) {
              setTouristPlaceList((pre) => {
                const newRestaurantList = [...pre];
                newRestaurantList[
                  touristPlaceList.indexOf((info) => info.id === _id)
                ] = res?.data;
                return newRestaurantList;
              });
              alert("তথ্য আপডেট হয়েছে");
            } else {
              setTouristPlaceList([...touristPlaceList, res.data]);
              alert("তথ্য যুক্ত হয়েছে");
            }
          } else {
            console.log(res.data);
          }
  ...
  const handleDelete = async (id) => {
    try {
      axios
        .delete(
          `${import.meta.env.VITE_APP_PUBLIC_SERVER}/api/touristspot/${id}`,
          {
            withCredentials: true,
          }
        )
        .then((res) => {
          if (res.status === 204) {
            const newRestaurantList = touristPlaceList.filter(
              (info) => info.id !== id
            );
            setTouristPlaceList(newRestaurantList);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  return (
    <>
      <h2 className="mb-6 text-center text-3xl font-bold text-gray-900">
        দর্শনীয় স্থান তালিকা
      </h2>
      {touristPlaceList.length > 0 ? (
      ...
      >
        <input
          ...
          placeholder="দর্শনীয় স্থান এর নাম"
        />
        <input
          required
          name="address"
